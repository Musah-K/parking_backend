  ...
  Query: {
    ...
    availableSlots: async (_, { available }) => {
      try {
        ...
        }).populate(available ? "" : "bookedBy"); // Populate only if false

        if (!slots.length) {
          throw new Error(
            `No ${available ? "available" : "booked"} parking slots found`
          );
        }
  ...
  Mutation: {
    ...
    removeExpiredSlots: async () => {
      try {
        ...
        const update = await ParkingSlot.updateMany(
          { validTill: { $lt: now }, isAvailable: false },
          {
            $set: {
              isAvailable: true,
              bookedBy: null,
